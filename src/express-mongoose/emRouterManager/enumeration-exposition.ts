import { EMRouterManager, EMSimpleController } from './emRouterManager';
import { Wrapper } from '../../hc-core/hcWrapper/hcWrapper';

function createEnumController(routerManager: EMRouterManager, name: string, enumerator: any): EMSimpleController;
function createEnumController(
  routerManager: EMRouterManager,
  name: string,
  enumerator: any,
  options: ExposeEnumerationOptions
): EMSimpleController;
function createEnumController(
  routerManager: EMRouterManager,
  name: string,
  ...
  let newController = new EMSimpleController(routerManager, resourceName);
  ...
  let getValueToExpose = (k, v) => {
    let valueToExpose = v;
    if (options && options.aliasSet && options.aliasSet.has(k)) valueToExpose = options.aliasSet.get(k);
    return valueToExpose;
  };

  let getCriteriaToExpose = k => {
    if (options && options.exposeCriteria && options.exposeCriteria.has(k)) return options.exposeCriteria.get(k);
    else return null;
  };
  ...
  newController.retrieveMethod = (req, res, next) => {
    ...
    let addDevData = i => {
      if (!devData) devData = { queryInconsistencies: [] };
      ...
      for (let p in req.query)
        switch (p) {
          case 'fixed_filter':
            ...
            let extractFilter = v => {
              let av = v.split('|');
              if (av.length == 3 && (av[1] == 'eq' || av[1] == '=')) return { key: av[0], value: av[2] };
              else addDevData(v);
            };

            if (queryValue instanceof Array) {
              (queryValue as Array<any>)
                .map(extractFilter)
                .filter(f => f != null)
                .forEach(f => equalFilters.set(f.key, f.value));
            } else {
    ...
    if (options && options.fixedExposedCriteria) equalFilters.set('criteria', options.fixedExposedCriteria);
    ...
    for (let element of arrayToExpose.filter(e => e != null)) {
      ...
      if (includeElement) filteredArray.push(element);
    }
...
export { createEnumController, ExposeEnumerationOptions };
