...
class EMRouterManager {
  ...
  exposeEntity<TDocument extends EntityDocument, TEntity extends EMEntity>(
    entityName: string,
    options: { controller?: EMEntityController<TDocument, TEntity>; basePath?: string; resourceName?: string }
  ): void;
  exposeEntity<TDocument extends EntityDocument, TEntity extends EMEntityMultiKey>(
    entityName: string,
    options: { controller?: EMEntityMutltiKeyController<TDocument, TEntity>; basePath?: string; resourceName?: string }
  ): void;
  exposeEntity<TDocument extends EntityDocument, TEntity extends EMEntityMultiKey>(
    entityName: string,
    options?: {
      controller?: EMEntityController<TDocument, TEntity> | EMEntityMutltiKeyController<TDocument, TEntity>;
      basePath?: string;
      resourceName?: string;
    }
  ): void {
    ...
    if (!entityController) {
      ...
      else entityController = new EMEntityController<TDocument, TEntity>(entityName, this, { resourceName });
    }
  ...
  exposeEnumeration(name: string, enumerator: any, options?: ExposeEnumerationOptions): void {
    ...
    let newController = createEnumController(this, name, enumerator, options);
  ...
  resolveComplexRetrieve(
    session: EMSession,
    construtorType: string,
    instanceId: string,
    ...
    constructionController.findEntity(session, instanceId).then(entity => {
      ...
      for (let i = 1; i < pathOverInstance.length; i++) {
        ...
        if (objectToExpose instanceof Array) objectToExpose = objectToExpose.find(obj => obj._id.toString() == nexStep);
        else objectToExpose = objectToExpose ? objectToExpose[pathOverInstance[i]] : null;
      }
      ...
      if (expositionAccessorInfo.activator.bindingType == MemberBindingType.Chunks) {
        ...
        if (isArray == null) isArray = expositionAccessorInfo.type == 'Array';

        if (isArray) expositionController.responseWrapper.entityCollection(session.response, objectToExpose);
        else expositionController.responseWrapper.entity(session.response, objectToExpose);
      }
    });
  }

  resolveComplexCreate(
    session: EMSession,
    construtorType: string,
    instanceId: string,
    expositionAccessorInfo: AccessorInfo,
    ...
    switch (expositionAccessorInfo.activator.bindingType) {
      ...
      case MemberBindingType.Snapshot:
        validateReqBody = () => {
          let expositionType: string;
          if (expositionAccessorInfo.activator instanceof EMMemberActivator)
            expositionType = (expositionAccessorInfo.activator as EMMemberActivator<EMEntity, EntityDocument>).entityInfo
              .name;
          if (expositionAccessorInfo.activator instanceof EMMemberTreeActivator)
            expositionType = expositionAccessorInfo.className;
        ...
        validateReqBody()
          .then(result => {
            ...
            let saveBaseEntity = objectToSend => {
              baseEntity.save().then(
                movFlow => {
                  if (movFlow.continue) {
                    if (objectToSend instanceof EMEntity)
                      constructionController.responseWrapper.entity(session.response, objectToSend);
                    else constructionController.responseWrapper.object(session.response, objectToSend);
                  } else constructionController.responseWrapper.logicError(session.response, movFlow.message);
                },
            ...
            if (result instanceof EMEntity) {
              if (expositionAccessorInfo.type == 'Array') {
                if (baseEntity[pathTo] == null) baseEntity[pathTo] = [];

                (baseEntity[pathTo] as Array<EMEntity>).push(result);
              } else baseEntity[pathTo] = result;

              saveBaseEntity(result);
            } else {
              if (!result.error) {
                this.saveEntityChunkMember(
                  session,
  ...
  resolveComplexUpdate(
    ...
    switch (expositionAccessorInfo.activator.bindingType) {
      ...
      case MemberBindingType.Chunks:
        validateReqBody = () =>
          this.genericValidation(session.request, { bindingType: MemberBindingType.Chunks, method: 'update' });
        break;
    }
          ...
          .then(result => {
              ...
              entityToSave.save().then(
                movFlow => {
                  ...
                  if (movFlow.continue) {
                    ...
                    else constructionController.responseWrapper.object(session.response, objectToSendInResponse);
                  } else constructionController.responseWrapper.logicError(session.response, movFlow.message);
                },
                error => constructionController.responseWrapper.exception(session.response, error)
              );
            ...
            if (result instanceof EMEntity) {
              if (expositionAccessorInfo.activator.bindingType == MemberBindingType.Snapshot) {
                if (expositionAccessorInfo.type == 'Array') {
                  let index = (baseEntity[pathTo] as Array<EMEntity>).findIndex(
                    e => e._id.toString() == result._id.toString()
                  );
                  if (index >= 0) (baseEntity[pathTo] as Array<EMEntity>)[index] = result;
                  else
                    constructionController.responseWrapper.handledError(
                      session.response,
                      `Index not found for item with id [${result._id.toString()}]`,
                      HttpStatus.BAD_REQUEST
                    );
                } else baseEntity[pathTo] = result;

                performSave(baseEntity, result);
              } else if (expositionAccessorInfo.activator.bindingType == MemberBindingType.Reference) performSave(result);
            } else {
  ...
  private saveEntityChunkMember(
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      let fileSize;
      let fileEntity;
      let member: string = expositionAccessorInfo.persistentAlias
        ? expositionAccessorInfo.persistentAlias
        : expositionAccessorInfo.name;
      let fileCollection: string = session.systemOwner.toLowerCase();
      if (option != 'create') {
      ...
      switch (option) {
        ...
        case 'delete': {
          let fileObjectId = new mongoose.Types.ObjectId(id);
          ...
          Promise.all([deleteChunksTask, deleteFileTask])
            .then(() => {
              if (expositionAccessorInfo.type == 'Array') {
                let index = baseEntity[member].findIndex(e => e._id == id);
                baseEntity[member].splice(index, 1);
              } else baseEntity[member] = null;
    ...
    constructionController
      ...
      .then(baseEntity => {
        let saveBaseEntity = () => {
          baseEntity.save().then(
            movFlow => {
              if (movFlow.continue) constructionController.responseWrapper.logicAccept(session.response, 'Record deleted');
              else constructionController.responseWrapper.logicError(session.response, movFlow.message);
            },
            error => constructionController.responseWrapper.exception(session.response, error)
          );
        ...
        if (expositionAccessorInfo.activator.bindingType == MemberBindingType.Chunks) {
          this.genericValidation(session.request, { bindingType: MemberBindingType.Chunks, method: 'delete' }).then(
            result => {
  ...
  getExpositionDetails(): Array<{ entityName: string; resourceName: string; basePath: string }> {
    return this._routers.map(r => {
      return { entityName: r.entityName, resourceName: r.controller.resourceName, basePath: r.basePath };
    });
  }

  findController<TEntity extends EMEntity, TDocument extends EntityDocument>(
    entityName: string
  ): EMEntityController<TDocument, TEntity> {
    return this._routers.find(ed => ed.entityName == entityName).controller;
  }

  genericValidation(
    request: express.Request,
    options?: { bindingType?: MemberBindingType; method?: string }
    ...
    return new Promise<GenericRequestValidation>((resolve, reject) => {
      if (options && options.bindingType == MemberBindingType.Chunks) {
        ...
        } else if (options.method && options.method == 'delete') resolve({ data: { ok: true } });
        else resolve({ error: 'File Handle Error' });
      }
  ...
  private getCompleteBasePath(postFix?: string): string {
    ...
    if (this._basePath) completeBasePath += this._basePath + '/';
  ...
  private getTemporalUnstrictModel(modelName: string) {
    if (!this._tempModels) this._tempModels = Array<{ modelName: string; model: mongoose.Model<any> }>();
...
class EMSimpleController {
  ...
  createRoutes(): void {
    ...
    if (this._retrieveByIdMethod)
      this._router.get('/' + this._resourceName + '/:_id', (request, response, next) =>
        this._retrieveByIdMethod(request, response, next)
      );
    ...
    if (this._deleteMethod)
      this._router.delete('/' + this._resourceName + '/:_id', (request, response, next) =>
        this._deleteMethod(request, response, next)
      );
  }
